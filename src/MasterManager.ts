...
export default class MasterManager {
    deleteContract(contract: Contract) {
        const targetName = contract.nodeName
        const targetNode = this.nodeManagerCollection[targetName]
        if (targetNode){
            targetNode.deleteContract(contract) 
        }
    }
    ...
    constructor(config: Configuration) {
        this.config = config;
        this.nodeManagerCollection = dictComp(
            (node) => [node.name, new NodeManager(node)],
            this.config.nodes
        );
    }
    ...
    registerContract(contract: Contract) {
        ...
        nodeManager.addContract(contract)
    }
}
