...
export class NodeManager {
  ...
  private addSocketListeners() {
    this.socket.on("data", (data) => this.io.emit("output", data.toString()));
  }

  private addIOListeners() {
    this.io.on("output", (data: string) => {
      if (data.startsWith("ENTER PASSWORD:")) return this.io.emit("enter-password");
    ...
    this.io.on("line", (data: string) => {
      if (data.startsWith(">")) this.io.emit("notification", data.slice(1));
    });
  ...
  private addNotifListeners() {
    for (const event of [EClientEvent.Connect, EClientEvent.Reauth, EClientEvent.Disconnect]) {
  ...
  writeClientDeny(CID: string, KID: string, reason: string = "Unauthorized", clientReason: string) {
